...
export function SADonations() {
  const { data: donations } = useApiList<Donation>("/donations", "super-admin");
  const total = donations.reduce((sum, donation) => sum + donation.amount, 0);
  const avgDonation = donations.length ? Math.round(total / donations.length) : 0;
  const donors = new Set(donations.map((donation) => donation.donor)).size;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Total Donors" value={donors} icon={<Users size={18} />} />
      </div>
